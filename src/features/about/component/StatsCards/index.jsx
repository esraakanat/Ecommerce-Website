...
export default function StatsCards() {
 ...
 "flex flex-col items-center pt-6 sm:pt-[22px] lg:pt-[26px]",
  hoveredCard === cardIndex ? "bg-[#DB4444]" : "bg-white",
    "transition-colors duration-300", ].join(" ");

  const ring ="w-16 h-16 sm:w-16 sm:h-16 rounded-full bg-black flex items-center justify-center";

  const getInnerCircleClass = (cardIndex) => [ "w-16 h-16 rounded-full flex items-center justify-center border-2 border-[#a3a3a3] transition-colors duration-300",
    hoveredCard === cardIndex ? "bg-white" : "bg-[#a3a3a3]"
  ].join(" ");

  const getBigTextClass = (cardIndex) => [ "font-semibold mt-4 text-[24px] sm:text-[26px] lg:text-[28px] leading-[30px] sm:leading-[32px] lg:leading-[34px]",
    hoveredCard === cardIndex ? "text-white" : "text-[#111111]",
    "transition-colors duration-300"
  ].join(" ");

  const getSubTextClass = (cardIndex) => [ "font-medium text-[11px] sm:text-[12px] leading-[16px] sm:leading-[18px] mt-[8px] sm:mt-[10px] text-center max-w-[200px]",
